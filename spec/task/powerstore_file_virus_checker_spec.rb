...
definition_name = 'powerstore_file_virus_checker'

describe "#{definition_name}" do

  it 'should perform file_virus_checker_collection_query' do
      task_name = 'powerstore::powerstore_file_virus_checker_collection_query'
  ...
  it 'should perform file_virus_checker_instance_query' do
      task_name = 'powerstore::powerstore_file_virus_checker_instance_query'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
  end  
  it 'should perform file_virus_checker_download_config_file' do
      task_name = 'powerstore::powerstore_file_virus_checker_download_config_file'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
  end  
  ...
  it 'should perform file_virus_checker_delete' do
      task_name = 'powerstore::powerstore_file_virus_checker_delete'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
	end  
