...
definition_name = 'powerstore_nfs_export'

describe "#{definition_name}" do

  it 'should perform nfs_export_collection_query' do
      task_name = 'powerstore::powerstore_nfs_export_collection_query'
  ...
  it 'should perform nfs_export_instance_query' do
      task_name = 'powerstore::powerstore_nfs_export_instance_query'
  ...
  it 'should perform nfs_export_delete' do
      task_name = 'powerstore::powerstore_nfs_export_delete'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
	end  
