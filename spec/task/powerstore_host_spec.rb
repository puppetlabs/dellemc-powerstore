...
definition_name = 'powerstore_host'

describe "#{definition_name}" do

  it 'should perform host_collection_query' do
      task_name = 'powerstore::powerstore_host_collection_query'
  ...
  it 'should perform host_instance_query' do
      task_name = 'powerstore::powerstore_host_instance_query'
  ...
  it 'should perform host_delete' do
      task_name = 'powerstore::powerstore_host_delete'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
	end  
