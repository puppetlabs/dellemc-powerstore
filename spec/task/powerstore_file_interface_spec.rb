...
definition_name = 'powerstore_file_interface'

describe "#{definition_name}" do

  it 'should perform file_interface_collection_query' do
      task_name = 'powerstore::powerstore_file_interface_collection_query'
  ...
  it 'should perform file_interface_instance_query' do
      task_name = 'powerstore::powerstore_file_interface_instance_query'
  ...
  it 'should perform file_interface_delete' do
      task_name = 'powerstore::powerstore_file_interface_delete'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
	end  
