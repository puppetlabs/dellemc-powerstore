...
definition_name = 'powerstore_remote_system'

describe "#{definition_name}" do
  it 'should perform remote_system_collection_query' do
    task_name = 'powerstore::powerstore_remote_system_collection_query'
  ...
  it 'should perform remote_system_instance_query' do
    task_name = 'powerstore::powerstore_remote_system_instance_query'
  ...
  it 'should perform remote_system_delete' do
    task_name = 'powerstore::powerstore_remote_system_delete'
    result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
    expect(result[0]['status']).to eq('success')  
	end
