...
definition_name = 'powerstore_snapshot_rule'

describe "#{definition_name}" do

  it 'should perform snapshot_rule_collection_query' do
      task_name = 'powerstore::powerstore_snapshot_rule_collection_query'
  ...
  it 'should perform snapshot_rule_instance_query' do
      task_name = 'powerstore::powerstore_snapshot_rule_instance_query'
  ...
  it 'should perform snapshot_rule_delete' do
      task_name = 'powerstore::powerstore_snapshot_rule_delete'
      result = run_task(task_name, 'prism', {"id" => "string"}, config: config_data, inventory: inventory_data)
      expect(result[0]['status']).to eq('success')  
	end  
