...
class Keystore_archiveDownloadakeystorebackuparchivefileTask < TaskHelper
  ...
  def task(arg_hash)
    ...
    operation_verb = 'Get'
    operation_path = '/api/rest/keystore_archive/%{filename}'
  ...
  if __FILE__ == $0
    Keystore_archiveDownloadakeystorebackuparchivefileTask.run
  end
